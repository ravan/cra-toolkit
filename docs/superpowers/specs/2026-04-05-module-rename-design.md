# Module Rename: cra-toolkit → cra-toolkit

**Date:** 2026-04-05

## Goal

Rename the Go module path from `github.com/ravan/cra-toolkit` to `github.com/ravan/cra-toolkit` to match the new GitHub repository name. The local directory name (`cra-toolkit`) is unchanged.
...
2. **Bulk replace** — `sed -i` across all `.go`, `.json`, `.yaml`, `.yml`, `.md` files, replacing `github.com/ravan/cra-toolkit` with `github.com/ravan/cra-toolkit`. Excludes `.worktrees/` and `.claire/` directories.
...
- Local directory rename (stays `cra-toolkit`)
