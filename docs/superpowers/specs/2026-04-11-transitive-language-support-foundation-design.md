...
- `ModulePath(file, sourceDir, packageName)` returns `packageName` unconditionally. This matches the current JavaScript behavior of always using flat keys keyed on the package name.
- `SymbolKey(modulePath, symbolName)` returns `modulePath + "." + symbolName`. Because `ModulePath` always returns `packageName`, the resulting key is always `packageName + "." + symbolName`, which is the current flat-key scheme.
